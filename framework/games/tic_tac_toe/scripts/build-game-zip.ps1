...
$FrameworkRoot = Split-Path -Parent (Split-Path -Parent $GameDir)

if (-not $OutZip) {
    $OutZip = Join-Path $GameDir "dist\tic_tac_toe.zip"
}

function Invoke-TicTacToeZipBuild {
    ...
    Push-Location (Join-Path $GameDir "web")
    try {
    ...
    $LogicSrc = Join-Path $FrameworkRoot "target\wasm32-wasip1\release\tic_tac_toe_component.wasm"
    if (-not (Test-Path $LogicSrc)) {
        throw "Missing $LogicSrc"
    }
