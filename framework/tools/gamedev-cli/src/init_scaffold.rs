...
use crate::{BackendKind, FrontendKind, InitArgs, JsTemplate, ProjectConfig};
...
fn scaffold_rust_bevy_flat(root: &Path, cfg: &ProjectConfig) -> Result<()> {
    ...
    let game_dep_path = find_framework_game_crate(root)
        .and_then(|game_crate| pathdiff::diff_paths(game_crate, &logic_dir))
        .unwrap_or_else(|| PathBuf::from("../../game"));
    let host_dep_path = find_framework_game_wasm_host_crate(root)
    ...
    let game_dep_path_bevy = find_framework_game_crate(root)
...
fn scaffold_tests_crate(root: &Path, cfg: &ProjectConfig) -> Result<()> {
    ...
    let game_dep_path_tests = find_framework_game_crate(root)
...
fn scaffold_rust_backend(root: &Path, cfg: &ProjectConfig, include_rust_frontend: bool) -> Result<()> {
    ...
    fs::write(
        shared_types_dir.join("src/bin/export_ts.rs"),
        export_ts,
    )?;
...
fn find_framework_game_crate(from: &Path) -> Option<PathBuf> {
...
fn find_framework_game_wasm_host_crate(from: &Path) -> Option<PathBuf> {
    let mut dir = Some(from);
    while let Some(current) = dir {
